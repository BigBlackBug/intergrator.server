...
public class IntegratorWorkerService {
	...
	getServiceInfo(ServiceDTO serviceDTO) {
		...
		if (endpointType == EndpointType.HTTP) {
			FullServiceDTO<HttpEndpointDescriptorDTO,
					HttpActionDTO> httpResult = new FullServiceDTO<HttpEndpointDescriptorDTO, HttpActionDTO>();
			...
			EndpointDTO<HttpEndpointDescriptorDTO> endpointDTO =
					new EndpointDTO<HttpEndpointDescriptorDTO>(endpointType, httpEndpointDescriptorDTO);
			...
			FullServiceDTO<JMSEndpointDescriptorDTO,
					QueueDTO> jmsResult = new FullServiceDTO<JMSEndpointDescriptorDTO, QueueDTO>();
			...
			EndpointDTO<JMSEndpointDescriptorDTO> endpointDTO =
					new EndpointDTO<JMSEndpointDescriptorDTO>(endpointType, httpEndpointDescriptorDTO);
	...
	getHttpActionDTOs(HttpServiceEndpoint service) {
		List<ActionEndpointDTO<HttpActionDTO>> result = new ArrayList<ActionEndpointDTO<HttpActionDTO>>();
	...
	getJmsActionDTOs(JMSServiceEndpoint service) {
		List<ActionEndpointDTO<QueueDTO>> result = new ArrayList<ActionEndpointDTO<QueueDTO>>();
		for (AbstractActionEntity action : service.getActions()) {
