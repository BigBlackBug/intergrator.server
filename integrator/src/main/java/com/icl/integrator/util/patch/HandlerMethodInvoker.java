...
public class HandlerMethodInvoker {
	...
	private static final Log logger = LogFactory
			.getLog(HandlerMethodInvoker.class);
	...
	public final Object invokeHandlerMethod(Method handlerMethod, Object handler,
		...
		Method handlerMethodToInvoke = BridgeMethodResolver
				.findBridgedMethod(handlerMethod);
		try {
			...
			for (Method attributeMethod : this.methodResolver.getModelAttributeMethods()) {
				...
				String attrName = AnnotationUtils
						.findAnnotation(attributeMethod, ModelAttribute.class).value();
				...
				if ("".equals(attrName)) {
					Class resolvedType = GenericTypeResolver
							.resolveReturnType(attributeMethodToInvoke,
							                   handler.getClass());
		...
		for (int i = 0; i < args.length; i++) {
			...
			for (Annotation paramAnn : paramAnns) {
				if (RequestParam.class.isInstance(paramAnn)) {
					...
					RequestBody requestHeader = (RequestBody) paramAnn;
					requestBodyFound = true;
					annotationsFound++;
					requestBodyRequired = requestHeader.required();
				}
			...
			if (paramName != null) {
				...
				args[i] = resolveRequestBody(methodParam, webRequest, requestBodyRequired);
			}
	 ...
	 */
	protected Object resolveRequestBody(MethodParameter methodParam, NativeWebRequest webRequest, boolean isRequired)
			throws Exception {

		return readWithMessageConverters(methodParam, createHttpInputMessage(webRequest), methodParam.getParameterType(),isRequired);
	}
	...
	private HttpEntity resolveHttpEntityRequest(MethodParameter methodParam, NativeWebRequest webRequest)
		...
		Object body = readWithMessageConverters(methodParam, inputMessage, paramType,true);
		return new HttpEntity<Object>(body, inputMessage.getHeaders());
	}

	private Object readWithMessageConverters(MethodParameter methodParam, HttpInputMessage inputMessage, Class paramType,boolean isRequired)
			throws Exception {
		long contentLength = inputMessage.getHeaders().getContentLength();

		if (!isRequired && contentLength == -1) {
			return null;
		}
		...
		if (type.getActualTypeArguments().length == 1) {
			...
			if (typeArgument instanceof Class) {
				...
				if (componentType instanceof Class) {
					...
					Object array = Array
							.newInstance((Class<?>) componentType, 0);
